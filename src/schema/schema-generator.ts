...
import { ClassMetadata, ParamMetadata, ResolverMetadata, SubscriptionResolverMetadata, } from '../metadata/definitions';
...
import { createAdvancedFieldResolver, createBasicFieldResolver, createHandlerResolver, } from '../resolvers/create';
...
export abstract class SchemaGenerator {
    ...
    static generateFromMetadataSync(options: SchemaGeneratorOptions): GraphQLSchema {
        ...
        const resolvers = BuildContext.router.resolvers;

        const orphanedTypes = options.orphanedTypes || (resolvers ? [] : undefined);
                ...
                type: new GraphQLObjectType({
                    ...
                    fields: () => {
                        ...
                        if ( objectType.interfaceClasses ) {
                            const interfacesFields = objectType.interfaceClasses.reduce<GraphQLFieldConfigMap<any, any>>((fieldsMap, interfaceClass) => {
    ...
    private static filterHandlersByRouter(method: string, handlers: ResolverMetadata[]): (ResolverMetadata | SubscriptionResolverMetadata)[] {
        ...
        const data = router.getRouterResolvers(method).map(route => {
            const handler = f(route, handlers, route.target);
            ...
            return {
                ...handler,
                schemaName: route.handleName
            };

        }).filter(x => !! x);

        return (data as (ResolverMetadata | SubscriptionResolverMetadata)[]);
    }
    ...
    private static buildRootQueryType(resolvers?: Function[]): GraphQLObjectType {
        ...
        const listHandlers = this.filterHandlersByRouter('query', getMetadataStorage().queries);

        return new GraphQLObjectType({
            name: 'Query',
            fields: this.generateHandlerFields(listHandlers),
        });
    }
    ...
    private static buildRootMutationType(resolvers?: Function[]): GraphQLObjectType | undefined {
        ...
        //     resolvers,
        // );
        const listHandlers = this.filterHandlersByRouter('mutation', getMetadataStorage().mutations);

        if ( listHandlers.length === 0 ) {
            return;
        }
    ...
    private static buildRootSubscriptionType(resolvers?: Function[]): GraphQLObjectType | undefined {
        const listHandlers = this.filterHandlersByRouter('subscription', getMetadataStorage().subscriptions);
        ...
        return new GraphQLObjectType({
            name: 'Subscription',
            fields: this.generateSubscriptionsFields(listHandlers as SubscriptionResolverMetadata[]),
        });
    }
    ...
    private static generateHandlerFields<T = any, U = any>(
        ...
        return handlers.reduce<GraphQLFieldConfigMap<T, U>>((fields, handler) => {
            // console.log(handler.resolverClassMetadata);
    ...
    private static filterHandlersByResolvers<T extends ResolverMetadata>(
        ...
        return resolvers ? handlers.filter(query => {
            return resolvers.includes(query.target);
        }) : handlers;
    }
