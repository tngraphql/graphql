...
import { Arg, ClassType, Field, InputType, Int, InterfaceType, ObjectType, Query, Resolver, } from '../../src';
...
describe('Generic types', () => {
    ...
    it('shouldn\'t emit abstract interface type', async () => {
        ...
        const { schemaIntrospection } = await getSchemaInfo({ router, container });
    ...
    describe('multiple children of base generic class', () => {
        ...
        beforeEach(async () => {
            ...
            const router = new Router();
            const container = new DefaultContainer();
            container.bind('GenericConnectionResolver', GenericConnectionResolver);
            router.query('users', 'GenericConnectionResolver.users');
            router.query('dogs', 'GenericConnectionResolver.dogs');
    ...
    describe('adding new properties in child class', () => {
        ...
        beforeEach(async () => {
            ...
            const router = new Router();
            const container = new DefaultContainer();
            container.bind('EdgeResolver', EdgeResolver);
            router.query('recipeEdge', 'EdgeResolver.recipeEdge');
            router.query('friendshipEdge', 'EdgeResolver.friendshipEdge');
    ...
    describe('overwriting a property from base generic class in child class', () => {
        ...
        beforeAll(async () => {
            ...
            const router = new Router();
            const container = new DefaultContainer();
            container.bind('OverwriteResolver', OverwriteResolver);
            router.query('child', 'OverwriteResolver.child');
